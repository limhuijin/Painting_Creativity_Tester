...
def predict_and_score(img_path):
    ...
    return predictions[0], total_score, score_category
...
for img_path in image_paths:
    predictions, total_score, score_category = predict_and_score(img_path)
    results.append([img_path, *predictions, total_score, score_category])

# 결과를 데이터프레임으로 변환
df = pd.DataFrame(results, columns=['Image Path', 'Delicate', 'Storytelling', 'Diversity of Objects', 'Utilize space', 'Expressive', 'Total Score', 'Category'])

# 엑셀 파일로 저장
output_path = 'C:/Users/user/Desktop/coding/Painting_Creativity_Tester/predictions.xlsx'
